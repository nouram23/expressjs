const ErrorBuilder = require("../utils/customError");
const asyncHandler = require("../middleware/asyncHandler");
const db = require("../config/firebase");

exports.createCarType = asyncHandler(async (req, res, next) => {
  if (!req.body.id || !req.body.mark) {
    throw new ErrorBuilder(
      "Машины төрөл үүсгэхэд шаардагдах үндсэн өгөгдөлүүдийг бүрэн бөглөнө үү?",
      404,
      false
    );
  }

  const response = await db.createCarType(req.body);
  res.status(200).json({
    status: 200,
    success: true,
    data: response,
  });
});

exports.createCar = asyncHandler(async (req, res, next) => {
  if (
    !req.body.isActive ||
    !req.body.carType ||
    !req.body.userId ||
    !req.file ||
    !req.body.carNo
  ) {
    throw new ErrorBuilder(
      "Үүсгэхэд шаардагдах үндсэн өгөгдөлүүдийг бүрэн бөглөнө үү?",
      404,
      false
    );
  }
  const imageUrl = await db.uploadImage(req.file);
  delete req.body.image;
  req.body.imageUrl = imageUrl;

  const response = await db.createCar(req.body);
  res.status(200).json({
    status: 200,
    success: true,
    data: response,
  });
});
exports.getCarTypes = asyncHandler(async (req, res, next) => {
  const data = await db.getCarTypes();
  res.status(200).json({
    status: 200,
    success: true,
    data: data,
  });
});
exports.getCars = asyncHandler(async (req, res, next) => {
  const data = await db.getCars();
  res.status(200).json({
    status: 200,
    success: true,
    data: data,
  });
});
exports.getOrderById = asyncHandler(async (req, res, next) => {
  const { id } = req.params;

  const documentSnapshot = await db.getOrderById(id);
  if (!documentSnapshot.exists()) {
    return res.status(404).json({
      status: 404,
      success: false,
      message: "Order not found",
    });
  }
  const orderData = documentSnapshot.data();
  res.status(200).json({
    status: 200,
    success: true,
    data: { id: documentSnapshot.id, ...orderData },
  });
});
exports.updateOrder = asyncHandler(async (req, res, next) => {
  const { id } = req.params;
  const { status } = req.body;

  if (status === undefined) {
    return res.status(400).json({
      status: 400,
      success: false,
      message: "Status is required",
    });
  }

  const response = await db.updateOrder(id, status);
  res.status(200).json({
    status: 200,
    success: true,
  });
});
